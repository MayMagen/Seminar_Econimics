...
main_regression <- lm(BuyingBarbie_category ~ Female + Age_over_50 + Versatile_category + EnvironmentEsteem_category + SelfEsteem_category + New_Barbie_Commercial, data = database)
...
reg4 <- lm(BuyingBarbie_category ~ Versatile_category + EnvironmentEsteem_category + SelfEsteem_category + Gush_Dan + Single +  HighIncome + New_Barbie_Commercial, data = database)
summary(reg4)
...
#Regression 4
tidy_model4 <- tidy(reg4, conf.int = TRUE)
...
#Regression 5
tidy_model5 <- tidy(reg5, conf.int = TRUE)
